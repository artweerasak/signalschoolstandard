...
export default function ImportQuestionsPage() {
  ...
  async function handleDeleteLibrary(libKey: string) {
    setDeleteLoading(true);
    setDeleteMsg("");
    try {
      const res = await fetch(`/military/api/v1/import/libraries/${libKey}/delete/`, {
        method: "DELETE",
        credentials: "include",
      ...
      setDeleteMsg("ลบ Library สำเร็จ");
      setDeleteConfirm(null);
      // reload libraries
      const newLibs = libraries.filter((l) => l.key !== libKey);
      setLibraries(newLibs);
      if (selectedLib === libKey) setSelectedLib(newLibs[0]?.key || "");
    } catch (e: any) {
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {libraries.length === 0 ? (
              ...
              <div className="flex gap-2 items-start">
                ...
                {selectedLib && (
                  <button
                    onClick={() => { setDeleteConfirm(selectedLib); setDeleteMsg(""); }}
                    className="px-3 py-2 bg-red-100 text-red-600 rounded-lg text-sm hover:bg-red-200 border border-red-200 whitespace-nowrap"
            ...
            {deleteMsg && (
              <p className={`text-sm mt-1 ${deleteMsg.startsWith("❌") ? "text-red-600" : "text-green-600"}`}>
                {deleteMsg}
              </p>
            )}
          </div>
          ...
          {deleteConfirm && (
            <div className="bg-red-50 border border-red-300 rounded-lg p-4">
              <p className="font-semibold text-red-800 mb-1">⚠️ ยืนยันการลบ Library</p>
              <p className="text-sm text-red-700 mb-3">
                คุณต้องการลบ <strong>{libraries.find(l => l.key === deleteConfirm)?.title}</strong> ใช่หรือไม่?
                การลบจะ<strong>ลบข้อสอบทั้งหมด</strong>ใน Library นี้ด้วย และไม่สามารถกู้คืนได้
              </p>
              <div className="flex gap-2">
                ...
                >
                  {deleteLoading ? "กำลังลบ..." : "ยืนยัน ลบ"}
                </button>
                <button
