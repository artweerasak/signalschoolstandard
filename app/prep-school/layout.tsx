...
export default function PrepSchoolLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    api.me()
      .then(u => {
        setUser(u)
        if (!u.is_staff && u.role !== "prep_school" && u.role !== "admin") {
          router.replace("/my")
        }
      })
    ...
    <div className="flex h-screen bg-[#f7f5fa]">
      <aside className="w-56 bg-gradient-to-b from-[#2D0F42] to-[#230a35] text-white flex flex-col flex-shrink-0">
        <div className="p-4 border-b border-white/10 flex items-center gap-2">
          ...
          <span className="font-bold text-sm leading-tight">แผนกเตรียมการ<br/>รร.สส.</span>
        </div>
