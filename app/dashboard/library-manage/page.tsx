...
export default function LibraryManagePage() {
  ...
  useEffect(() => {
    ...
    setChecked(new Set());
    setMsg(null);
    fetch(`/military/api/v1/import/libraries/${encodeURIComponent(selectedLib)}/blocks/`, {
      credentials: "include",
    })
      .then((r) => r.json())
      .then((d) => {
  ...
  async function handleBulkDelete() {
    ...
    try {
      ...
      setMsg({
        type: "success",
        text: `✅ ลบสำเร็จ ${data.deleted} ข้อ${data.errors?.length > 0 ? ` (เกิดข้อผิดพลาด ${data.errors.length} ข้อ)` : ""}`,
      });
      // Remove deleted blocks from list
      const deletedKeys = new Set(usageKeys);
      setBlocks((prev) => prev.filter((b) => !deletedKeys.has(b.usage_key)));
      setChecked(new Set());
    } catch (e: any) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          href="/dashboard/import-questions"
      ...
      {loadingLibs ? (
        ...
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg p-4">
          ...
          <Link href="/dashboard/import-questions" className="underline text-blue-600">
            นำเข้าข้อสอบก่อน
          </Link>
        </div>
      ) : (
